...
#pragma omp parallel for default(none) private(min_distance, distance, cluster_id) shared(num_points, num_clusters, num_dimensions, dataset, centroids)
        for (auto i = 0; i < num_points; i++) {
        ...
        if (!first_iteration && check_equal_clusters3(dataset, old_dataset, num_points)) {
            convergence = true;
        } else {
            old_dataset = dataset;
